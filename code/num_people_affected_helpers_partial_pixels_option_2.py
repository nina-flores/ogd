...
def add_bounding_box_col(ch_df: gpd.GeoDataFrame, target_col: str, buffer_buffer: int = 0):
    # Initialize a list to store bounding boxes
    bounding_boxes = []
    
    # Iterate through rows of the GeoDataFrame
    for index, row in tqdm(ch_df.iterrows(), total=len(ch_df), desc="Adding bounding box (5/6)"):
        target_col_geom = row[target_col]
        bounding_box = target_col_geom.envelope.buffer(buffer_buffer)
        bounding_boxes.append(bounding_box)
    
    # Add the bounding boxes to the GeoDataFrame
    ch_df["bounding_box"] = bounding_boxes
    return ch_df
